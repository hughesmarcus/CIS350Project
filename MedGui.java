...
public class MedGui {
	...
	/**
	 * list that contais all the users who have access to DB.
	 */
	static JList userList;
	
	/**
	 * jpanel that has the profiles included.
	 */
	private JPanel profilePatPanel;
	
	/**
	 * jpanel with doctors profiles included.
	 */
	private JPanel docProfilePanel;
	
	/**
	 * panel used to deliver messages.
	 */
	private JPanel messagePanel;
	
	/**
	 * panel used to edit the database.
	 */
	private JPanel dbEditPanel;
	
	/**
	 * the panel which the user edits information.
	 */
	private JPanel userEditPanel;
	
	/**
	 * login dialog that is the same message depending on what
	 * user is loging in.
	 */
	private static LoginDialog login;
	
	/**
	 * the box that holds the list of all the symptoms
	 * currently in the DB.
	 */
	private JComboBox sympBox;
	
	/**
	 * instance of doctor used inside this class.
	 */
	private Doctor doc;
	
	/**
	 * instance of patient used inside this class.
	 */
	private Patient pat;
	
	/**
	 * instance of admin used in this class.
	 */
	private Admin ad;
	
	/**
	 * text area that holds results of a user's action.
	 */
	private JTextArea resultArea;
	
	/**
	 * combo box that allows for convenient searches
	 * of a symptom.
	 */
	private JComboBox searchSympBox;
	
	/**
	 * patient list that is pulled up for either admin or doctors.
	 */
	private JList patList;
	
	/**
	 * text area that holds the information of a patient.
	 */
	private JTextArea patInfoArea;
	
	/**
	 * message list that is used for interaction between doctor and patient. 
	 */
	private JList messageList;
	
	/**
	 * model used for the messaging feature.
	 */
	DefaultListModel messageModel;
	
	/**
	 * text are that supports entering text for message feature.
	 */
	private JTextArea messageArea;
	
	/**
	 * user id number to keep track of who accesses database.
	 */
	private int userID;
	
	/**
	 * string that holds the message to be sent.
	 */
	private String sendMessageTo;
	
	/**
	 * type of user that is using the database.
	 */
	private String userType;
	
	/**
	 * string that holds the username of the user accessing database.
	 */
	private String userName;
	
	/**
	 * list that holds the medications for the illnesses.
	 */
	private DefaultListModel allMedsModel;
	
	/**
	 * meds that one particular patient has.
	 */
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					login.setIconImage(Toolkit.getDefaultToolkit().getImage("C:\\Users\\silas\\Desktop\\Java\\MediApp2\\mediicon.jpg"));
	 ...
	 * @param type the string used to tell what type the user is
	 * @param userN the name of the user (their login name)
	 */
	public MedGui(final String type, final String userN) {
		userType = type;
		userName = userN;
		initialize();
	}
	...
	private void initialize() {
		...
		final JList sympsList = new JList();
		sympsList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		final JList illsList = new JList(illnessList);
		...
		final JTextArea userInfoArea = new JTextArea();
		...
		userList.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(final ListSelectionEvent arg0) {
				...
				if (type.equals("A")) {
					type = "Admin";
					//if user is an admin get the admin 
					//info(type, id, fname, lname, pass)
					...
					//if user is a doctor get doctor 
					//info(type, id, fname, lname, pass, spec, patients)
					type = "Doctor";
					int id = DB.getID(selUser);
					String name = DB.getName(id, type);
					String password = DB.getPass(id);
					...
					//if user is a patient get patient
					//info(type, id, fname, lname, height, weight, insur, symptoms, doctors)
					type = "Patient";
					int id = DB.getID(selUser);
					String name = DB.getName(id, type);
					String password = DB.getPass(id);
	...
	/**
	 * method to update an illness' symptom list.
	 */
	public final void updateSympList() {
	...
	/**
	 * method that updates the messages sent to the user.
	 * @param id the id for the user accessing database.
	 * @param type type of user that accesses the database.
	 */
	public final void updateMessages(final int id, final String type) {
	...
	/**
	 * method that is going to update all the users on the list
	 * of possible users.
	 */
