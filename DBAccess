...
public class DBAccess 
{
	...
	public String login(String user, String pass)
	{
		...
		{ 
			...
			ResultSet res = st.executeQuery("SELECT username, password, type FROM users WHERE username = '" + user + "' AND password = '" + pass + "';"); 
			while (res.next()) 
			{
	 ...
	 * an arraylist of all the illnesses with that symptom
	 * @param symptom
	 * @return illness
	 */
	public ArrayList<String> sympSearch(String symptom)
	{
		...
		{ 
			...
			Connection conn = DriverManager.getConnection(url+dbName,userName,password); 
			Statement st = conn.createStatement();
			ResultSet res = st.executeQuery("SELECT Illness_Name FROM illness WHERE Symptom = '" + symptom + "';"); 
			while (res.next()) 
			{
	...
	public ArrayList<String> illSearch(String ill)
	{
		...
		{ 
			...
			Connection conn = DriverManager.getConnection(url+dbName,userName,password); 
			Statement st = conn.createStatement();
			ResultSet res = st.executeQuery("SELECT Symptom FROM illness WHERE Illness_Name = '" + ill + "';"); 
			while (res.next()) 
			{
	 ...
	 * returns an arrayList of rejected symptoms.
	 * @param ill
	 * @param symps
	 * @return reject
	 */
	public ArrayList<String> addIll(String ill, ArrayList<String> symps)
	{
		...
		{ 
			...
			for(int i = 0; i < symps.size(); i++)
			{
				try
				{
					st.executeUpdate("INSERT INTO illness VALUES('" + ill + "', '" + symps.get(i) + "');"); 
				}
	...
	public ArrayList<String> getIlls()
	{
		...
		{ 
			...
			Connection conn = DriverManager.getConnection(url+dbName,userName,password); 
			Statement st = conn.createStatement();
			ResultSet res = st.executeQuery("SELECT DISTINCT Illness_Name FROM illness;"); 
			while (res.next()) 
			{
	...
	public ArrayList<String> getSymps(String ill)
	{
		...
		{ 
			...
			Connection conn = DriverManager.getConnection(url+dbName,userName,password); 
			Statement st = conn.createStatement();
			ResultSet res = st.executeQuery("SELECT DISTINCT Symptom FROM illness WHERE Illness_Name = '" + ill + "';"); 
			while (res.next()) 
			{
	...
	/**
	 * Searches for the given illness and symptom then deletes the symptom, returns
	 * true if successful
	 * @param ill
	 * @param symp
	 * @return del
	...
	public boolean delSymp(String ill, String symp)
	{
		...
		{ 
			...
			try
			{
				st.executeUpdate("DELETE FROM illness WHERE Illness_Name = '" + ill +"' AND Symptom = '" + symp +"';"); 
				del = true;
			}
			catch(com.mysql.jdbc.exceptions.jdbc4.MySQLIntegrityConstraintViolationException err)
			{
	...
	public boolean delIll(String ill)
	{
		...
		{ 
			...
			try
			{
				st.executeUpdate("DELETE FROM illness WHERE Illness_Name = '" + ill +"';"); 
				del = true;
			}
			catch(com.mysql.jdbc.exceptions.jdbc4.MySQLIntegrityConstraintViolationException err)
			{
